...
local MENU_BUTTON_HEIGHT = 36;
local pairs, ipairs, table, mrandom, CreateFont = _G.pairs, _G.ipairs,  _G.table, _G.math.random, _G.CreateFont;
...
local HandleTabButtonOnLeave;

local function EnsureConfigFontObjects()
  local fontTemplates = {
    { "MUI_ConfigFont", "GameFontHighlight" };
    { "MUI_ConfigFontSmall", "GameFontNormalSmall" };
    { "MUI_ConfigFontLarge", "GameFontHighlightLarge" };
    { "MUI_ConfigFontDisabled", "GameFontDisable" };
  };

  for _, fontInfo in ipairs(fontTemplates) do
    local fontName = fontInfo[1];
    local templateName = fontInfo[2];
    local fontObject = _G[fontName];

    if (not fontObject) then
      fontObject = CreateFont(fontName);
    end

    if (obj:IsTable(fontObject) and not fontObject.__muiConfigFontInitialized) then
      local templateFont = _G[templateName];

      if (obj:IsTable(templateFont) and obj:IsFunction(templateFont.GetFont)) then
        local fontPath, size, flags = templateFont:GetFont();
        fontObject:SetFont(fontPath, size, flags);

        if (obj:IsFunction(templateFont.GetTextColor) and obj:IsFunction(fontObject.SetTextColor)) then
          local r, g, b, a = templateFont:GetTextColor();
          fontObject:SetTextColor(r, g, b, a or 1);
        end

        if (obj:IsFunction(templateFont.GetShadowColor) and obj:IsFunction(fontObject.SetShadowColor)) then
          local sr, sg, sb, sa = templateFont:GetShadowColor();
          fontObject:SetShadowColor(sr, sg, sb, sa or 0);
        end

        if (obj:IsFunction(templateFont.GetShadowOffset) and obj:IsFunction(fontObject.SetShadowOffset)) then
          local sx, sy = templateFont:GetShadowOffset();
          fontObject:SetShadowOffset(sx or 0, sy or 0);
        end
      end

      fontObject.__muiConfigFontInitialized = true;
    end
  end
end
...
do
  ...
  function CreateTopMenuButton(label, onClick, anchor)
    local btn = gui:CreateButton(nil, label, nil, nil, 30, 110);
...
local function IsMenuVisibleInCurrentPhase(menuConfigTable)
  if (not obj:IsTable(menuConfigTable)) then
    return false;
  end

  if (not obj:IsString(menuConfigTable.featurePath) or menuConfigTable.featurePath == tk.Strings.Empty) then
    return true;
  end

  return FeatureState:IsEnabled(menuConfigTable.featurePath);
end

local function ApplyThemeToFrameTree(frame)
  if (not (obj:IsTable(frame) and obj:IsFunction(frame.GetChildren))) then
    return;
  end
  ...
  for _, child in ipairs(children) do
    if (obj:IsTable(child.dropdown) and obj:IsFunction(child.dropdown.ApplyThemeColor)) then
      child.dropdown:ApplyThemeColor();
    elseif (obj:IsFunction(child.ApplyThemeColor)) then
      child:ApplyThemeColor();
    end

    ApplyThemeToFrameTree(child);
  end
end
...
function C_ConfigMenuModule:RenderMenuComponents(_, menuButton)
  ...
  if (not obj:IsTable(menuConfig)) then
    return
  end

  if (obj:IsFunction(menuConfig.children)) then
    local ok, childrenOrError = pcall(menuConfig.children, menuConfig);

    if (ok) then
      menuConfig.children = childrenOrError;
    else
      MayronUI:LogError(strformat(
        "[MUI_Config] Failed to build menu children for '%s': %s",
        tostring(menuButton.name or menuConfig.name or "<unknown menu>"),
        tostring(childrenOrError)));
      return
    end
  end
  ...
  for _, componentConfig in pairs(menuConfig.children) do
    if (obj:IsTable(componentConfig) and not IsUnsupportedByClient(componentConfig.client)) then
      local ok, err = pcall(self.RenderComponent, self, menuConfig, componentConfig, radioGroups);

      if (not ok) then
        MayronUI:LogError(strformat(
          "[MUI_Config] Failed to render component for '%s': %s",
          tostring(menuButton.name or menuConfig.name or "<unknown menu>"),
          tostring(err)));
      end
    end
...
function C_ConfigMenuModule:RefreshThemeVisuals(data, skipMenuRefresh)
  if (not data.configPanel) then
    return;
  end

  local backButton = data.configPanel.back;

  if (backButton) then
    local normalTexture = backButton:GetNormalTexture();
    local highlightTexture = backButton:GetHighlightTexture();

    if (normalTexture and highlightTexture) then
      tk:ApplyThemeColor(normalTexture, highlightTexture);
    elseif (backButton.arrow) then
      tk:ApplyThemeColor(backButton.arrow);
    end
  end

  if (data.configPanel.profilesBtn and obj:IsFunction(data.configPanel.profilesBtn.ApplyThemeColor)) then
    data.configPanel.profilesBtn:ApplyThemeColor();
  end

  if (data.configPanel.layoutsBtn and obj:IsFunction(data.configPanel.layoutsBtn.ApplyThemeColor)) then
    data.configPanel.layoutsBtn:ApplyThemeColor();
  end

  if (data.configPanel.refreshButton) then
    local refreshTexture = data.configPanel.refreshButton:GetNormalTexture();

    if (refreshTexture) then
      tk:ApplyThemeColor(refreshTexture);
    end
  end

  if (obj:IsTable(data.menuButtons)) then
    for _, menuButton in ipairs(data.menuButtons) do
      local normal = menuButton:GetNormalTexture();
      local highlight = menuButton:GetHighlightTexture();
      local checked = menuButton:GetCheckedTexture();

      if (normal and highlight) then
      ...
      if (menuButton.tabsContainer and obj:IsTable(menuButton.tabsContainer.tabs)) then
        for _, tab in ipairs(menuButton.tabsContainer.tabs) do
  ...
  if (data.selectedButton and data.selectedButton.menu) then
    ApplyThemeToFrameTree(data.selectedButton.menu:GetFrame());

    if (not skipMenuRefresh) then
      data.selectedButton.menu:Refresh();
    end
  end
end

HandleTabButtonOnLeave = function(self)
...
function C_ConfigMenuModule:RenderMenuTabs(_, menuButton, menuParent)
  ...
  for i, tabChildren in ipairs(menuButton.configTable.children) do
    ...
    local fs = tab:CreateFontString(nil, "OVERLAY", "MUI_ConfigFont", 20);
    fs:SetJustifyH("LEFT");
    fs:SetText(tabNames[i]);

    local tabWidth = fs:GetStringWidth() + 60;
...
function C_ConfigMenuModule:SetUpWindow(data)
  ...
  configFrame:Hide();
  configFrame:SetSize(1120, 620);
  configFrame:SetPoint("CENTER");
  configFrame:EnableMouse(true);
  tk:SetResizeBounds(configFrame, 860, 520, 1500, 900);
  ...
  data.configPanel:GetColumn(1):SetFixed(230);
  data.configPanel:GetRow(1):SetFixed(72);
  data.configPanel:GetRow(3):SetFixed(44);
  ...
  menuListCell:SetInsets(0, 12, 0, 10);

  data.options = data.configPanel:CreateCell();
  data.options:SetInsets(6, 12, 6, 6);

  local versionCell = data.configPanel:CreateCell();
  versionCell:SetInsets(8, 12, 8, 10);

  versionCell.text = versionCell:CreateFontString(nil, "OVERLAY", "MUI_ConfigFont");
  ...
  data.warningLabel = bottombar:CreateFontString(nil, "OVERLAY", "MUI_ConfigFont");
  ...
  data.configPanelName = topbar:CreateFontString(nil, "ARTWORK", "MUI_ConfigFontLarge");
  ...
  data.configPanel.refreshButton = refreshButton;
...
do
  ...
  local function AddMenuButton(menuButtons, menuConfigTable, menuListFrame)
    if (IsUnsupportedByClient(menuConfigTable.client) or not IsMenuVisibleInCurrentPhase(menuConfigTable)) then
      return
    end
    ...
    menuButton.text = menuButton:CreateFontString(nil, "OVERLAY", "MUI_ConfigFont");
    ...
    -- This should not happen if client gating is configured correctly.
  ...
  function C_ConfigMenuModule:SetUpSideMenu(data, menuListFrame)
    data.menuButtons = {};
    local nextGeneratedMenuId = 100;

    for _, module in MayronUI:IterateModules() do
      if (module.GetConfigTable) then
        local ok, configTableOrError = pcall(module.GetConfigTable, module, self);

        if (ok and configTableOrError == nil) then
          -- module intentionally has no config for this client/version
        elseif (ok and obj:IsTable(configTableOrError)) then
          local configTable = configTableOrError;

          if (#configTable == 0) then
            if (configTable.id == nil) then
              configTable.id = nextGeneratedMenuId;
              nextGeneratedMenuId = nextGeneratedMenuId + 1;
            end

            AddMenuButton(data.menuButtons, configTable, menuListFrame);
          elseif (#configTable > 0) then
            -- ConfigModule has multiple menu buttons
            for _, menuConfigTable in ipairs(configTable) do
              if (obj:IsTable(menuConfigTable)) then
                if (menuConfigTable.id == nil) then
                  menuConfigTable.id = nextGeneratedMenuId;
                  nextGeneratedMenuId = nextGeneratedMenuId + 1;
                end
          ...
          local moduleName = module.GetModuleKey and module:GetModuleKey() or tostring(module);
          MayronUI:LogError(strformat(
            "[MUI_Config] Failed to load GetConfigTable for '%s': %s",
            tostring(moduleName or "<unknown module>"),
            tostring(configTableOrError)));
        end
      end
    end
