...
local function SetAddOnProfilePair(_, data, addOnName, profileName)
  local layoutData = LayoutManager:GetLayoutData(data.viewingLayout);
...
local function GetSupportedAddOns()
  ...
  for _, database in MayronDB.Static:IterateDatabases() do
    ...
    if (dbName ~= "MayronUI") then
      -- MayronUI now owns its own full profile per layout, so the layout tool
      -- only manages external addon profile assignments here.
...
local function UpdateLayoutProfileLabel(data)
  if (data.layoutProfileValue) then
    data.layoutProfileValue:SetText(LayoutManager:GetProfileName(data.viewingLayout));
  end
end

local function IterateLayouts()
  return LayoutManager:IterateLayouts();
end
...
local function LayoutExists(layoutName)
  return LayoutManager:LayoutExists(layoutName);
end
...
function C_LayoutSwitcher:SetViewingLayout(data, dropdown, layoutName)
  if (not layoutName) then
    if (dropdown) then
      ...
      for key, layoutData in IterateLayouts() do
        if (layoutData) then
          layoutName = key;
          break
        end
  ...
  if (LayoutExists(layoutName)) then
    UpdateLayoutProfileLabel(data);
...
function C_LayoutSwitcher:GetNumLayouts()
  ...
  for _, layoutData in IterateLayouts() do
    if (layoutData) then
      n = n + 1;
    end
  end
...
function C_LayoutSwitcher:UpdateAddOnWindow(data)
  ...
  local layoutData = GetLayoutData(data.viewingLayout);
...
function C_LayoutSwitcher:CreateNewAddOnProfile(data, dropdown, addOnName, dbObject)
  ...
  dropdown:SetLabel(GetLayoutData(data.viewingLayout)[addOnName]);
...
function C_LayoutSwitcher:CreateLayout(data)
  ...
  _G.StaticPopupDialogs["MUI_CreateLayout"] = {
    ...
    OnAccept = function(dialog)
      ...
      if (not LayoutExists(layout)) then
        LayoutManager:SetLayoutData(layout, obj:PopTable());
        LayoutManager:SetProfileName(layout, LayoutManager:GetCanonicalProfileName(layout));
...
function C_LayoutSwitcher:RenameLayout(data)
  ...
  _G.StaticPopupDialogs["MUI_RenameLayout"] = {
    ...
    OnAccept = function(dialog)
      ...
      if (LayoutExists(layout)) then
        return;
      end

      local oldViewingLayout = data.viewingLayout;
      local oldProfileName = LayoutManager:GetProfileName(oldViewingLayout);
      local defaultOldProfileName = LayoutManager:GetCanonicalProfileName(oldViewingLayout);
      local renamedProfileName = oldProfileName == defaultOldProfileName and LayoutManager:GetCanonicalProfileName(layout)
        or oldProfileName;

      data.viewingLayout = layout;
      LayoutManager:RenameLayout(oldViewingLayout, layout);
      LayoutManager:SetProfileName(layout, renamedProfileName);
      LayoutManager:SetProfileName(oldViewingLayout, false);
...
function C_LayoutSwitcher:DeleteLayout(data)
  ...
  _G.StaticPopupDialogs["MUI_DeleteLayout"] = {
    ...
    OnAccept = function()
      ...
      LayoutManager:DeleteLayout(data.viewingLayout);
      LayoutManager:SetProfileName(data.viewingLayout, false);
...
function C_LayoutSwitcher:ShowLayoutTool(data)
  ...
  data.description.text = data.description:CreateFontString(nil, "ARTWORK", "MUI_ConfigFont");
  ...
  for addOnName, dbObject in pairs(data.supportedDatabases) do
    local cbContainer, dropdown = self:CreateScrollFrameRowContent(data, dbObject, addOnName);
  ...
  data.layoutsTitle = layoutsDropdownContainer:CreateFontString(nil, "OVERLAY", "MUI_ConfigFont");
  ...
  data.layoutProfileTitle = actionsFrame:CreateFontString(nil, "OVERLAY", "MUI_ConfigFont");
  data.layoutProfileTitle:SetText(L["Profile"] .. ":");
  data.layoutProfileTitle.fullWidth = true;

  data.layoutProfileValue = actionsFrame:CreateFontString(nil, "OVERLAY", "MUI_ConfigFontLarge");
  data.layoutProfileValue.fullWidth = true;
  UpdateLayoutProfileLabel(data);
  ...
  actionsDynamicFrame:AddChildren(
    layoutsDropdownContainer,
    data.layoutProfileTitle,
    data.layoutProfileValue,
    data.createButton,
    data.renameButton,
    data.deleteButton
  );
