...
local function HandleDropdownReset(self, value)
  ...
  if (obj:IsTable(configOptions)) then
    if (self.usesValueLabels and obj:IsString(value) and obj:IsString(configOptions[value])) then
      dropdown:SetLabel(configOptions[value]);
      return;
    end

    for key, optionValue in pairs(configOptions) do
      if (obj:IsTable(optionValue)) then
        local pair = optionValue;
        key = pair[1];
        optionValue = pair[2];
      end

      if (optionValue == value or key == value) then
        dropdown:SetLabel(tostring(self.usesValueLabels and optionValue or key));
        return;
      end
    end
  end

  dropdown:SetLabel(tostring(value or tk.Strings.Empty));
end
...
function Components.dropdown(parent, config, value)
  ...
  dropdownFrame.configOptions = options;
  dropdownFrame.usesValueLabels = (config.labels == "values");
