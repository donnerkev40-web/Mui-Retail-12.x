...
function Utils:WrapInNamedContainer(component, config)
  ...
  container.name = container:CreateFontString(nil, "OVERLAY", "MUI_ConfigFont");
  ...
  if (config.inline) then
    local componentHeight = component:GetHeight();
    local labelHeight = container.name:GetStringHeight();
    local rowHeight = _G.math.max(componentHeight, labelHeight, 22);

    container.fullWidth = true;
    container:SetHeight(rowHeight);

    container.name:ClearAllPoints();
    container.name:SetPoint("LEFT", container, "LEFT", 0, 0);
    container.name:SetPoint("RIGHT", container, "RIGHT", -((currentWidth or 0) + componentNameSpacing), 0);
    container.name:SetJustifyH("LEFT");

    component:ClearAllPoints();

    if (container.reset) then
      container.reset:ClearAllPoints();
      container.reset:SetPoint("RIGHT", container, "RIGHT", 0, 0);
      container.reset:SetPoint("TOP", container, "TOP", 0, 0);
      component:SetPoint("TOPRIGHT", container.reset, "TOPLEFT", -4, 0);
      container.name:SetPoint("RIGHT", container.reset, "LEFT", -((currentWidth or 0) + componentNameSpacing + 4), 0);
    else
      component:SetPoint("TOPRIGHT", container, "TOPRIGHT", 0, 0);
    end
  else
...
function Utils:AppendDefaultValueToTooltip(config, dropdownOptions)
  ...
  local defaultTextValue = default;
  ...
  if (canReset and doesNotContainDefaultTooltip) then
    if (dropdownOptions and obj:IsTable(dropdownOptions)) then
      for key, value in pairs(dropdownOptions) do
        ...
        if (obj:IsString(key)) then
          if (config.labels == "values" and key == default) then
            defaultTextValue = value;
            break
          elseif (value == default) then
            defaultTextValue = key; -- use the key of the DropDown component to represent the name of the default value to the user
            break
          end
        end
      end
    ...
    local defaultText;
    if (obj:IsBoolean(defaultTextValue)) then
      defaultText = defaultTextValue and L["Enabled"] or L["Disabled"];
    elseif (config.defaultText) then
      defaultText = config.defaultText;
    else
      defaultText = tostring(defaultTextValue);
    end
