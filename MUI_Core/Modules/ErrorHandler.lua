...
local seterrorhandler, geterrorhandler = _G.seterrorhandler, _G.geterrorhandler;
...
function C_ErrorHandler:OnInitialize(data)
  ...
  if (obj:IsFunction(geterrorhandler)) then
  ...
  seterrorhandler(function(errorMessage)
    ...
    if (not MayronUI.PauseErrors) then
      local luaErrorHandler = HandleLuaError or _G.HandleLuaError;

      if (obj:IsFunction(luaErrorHandler)) then
        luaErrorHandler(errorMessage);
      elseif (obj:IsFunction(previousErrorHandler)) then
        previousErrorHandler(errorMessage);
      end
    end
  end);
  ...
  -- BugGrabber integration is currently disabled because recent BugGrabber
  -- builds in this environment no longer expose a stable RegisterCallback API,
  -- which causes the entire ErrorHandler module to fail during initialization.
end
