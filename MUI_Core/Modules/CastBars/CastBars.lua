...
local issecretvalue, issecrettable = _G.issecretvalue, _G.issecrettable;
...
local LibCC;
local GetSafeCastInfo;

local function GetPlayerCastingBarFrame()
  return _G.PlayerCastingBarFrame or _G.CastingBarFrame;
end

local function IsCastBarFrameReady(castBarData)
  return obj:IsTable(castBarData)
    and obj:IsWidget(castBarData.frame)
    and obj:IsWidget(castBarData.frame.statusbar);
end
...
local function SuppressBlizzardCastBar(frame)
  ...
  if (tk:IsRetail()) then
    if (not frame.__muiCastBarSuppressed) then
      frame.__muiCastBarSuppressed = true;
      frame:UnregisterAllEvents();
      frame.Show = tk.Constants.DUMMY_FUNC;
...
local function ShouldHideBlizzardCastBars()
  return not not (db.global.castBars and db.global.castBars.hideBlizzard);
end
...
function Events:UNIT_SPELLCAST_STOP(castBar, castBarData, unitID)
  if (GetSafeCastInfo(castBarData.unitID, false)) then
    self:UNIT_SPELLCAST_START(castBar, castBarData, unitID);
    return
  end

  if (GetSafeCastInfo(castBarData.unitID, true)) then
...
function Events:PLAYER_TARGET_CHANGED(castBar, castBarData)
  ...
  if (UnitExists(castBarData.unitID)) then
    ...
    if (GetSafeCastInfo(castBarData.unitID, false)) then
      ...
      active = true;
    elseif (GetSafeCastInfo(castBarData.unitID, true)) then
...
local UnitBuff, tostring = _G.UnitBuff, _G.tostring;
local BUFF_MAX_DISPLAY = _G.BUFF_MAX_DISPLAY or 40;

if (not obj:IsFunction(UnitBuff)) then
  UnitBuff = function(unitID, auraID, filter)
    local UnitAura = _G.UnitAura;

    if (obj:IsFunction(UnitAura)) then
      local auraFilter = "HELPFUL";

      if (obj:IsString(filter) and filter ~= "") then
        auraFilter = auraFilter .. "|" .. filter;
      end

      return UnitAura(unitID, auraID, auraFilter);
    end
  end
end
...
local function GetSafeNumericValue(value, fallback)
  ...
  local ok, safeValue = _G.pcall(function()
    return value + 0;
  end);

  if (ok and obj:IsNumber(safeValue)) then
    return safeValue;
  end
...
local function GetAuraDataByIndex(unitID, auraIndex)
  if (not (tk:IsRetail() and C_UnitAuras and C_UnitAuras.GetAuraDataByIndex)) then
    return;
  end

  local ok, auraData = _G.pcall(C_UnitAuras.GetAuraDataByIndex, unitID, auraIndex, "HELPFUL");

  if (ok and obj:IsTable(auraData) and NotSecretTable(auraData)) then
    return auraData;
  end
end

local function GetHelpfulAuraInfo(unitID, auraIndex)
  local auraData = GetAuraDataByIndex(unitID, auraIndex);

  if (auraData) then
    return
      NotSecretValue(auraData.name) and auraData.name or nil,
      NotSecretValue(auraData.icon) and auraData.icon or nil,
      GetSafeNumericValue(auraData.duration),
      GetSafeNumericValue(auraData.expirationTime),
      GetSafeNumericValue(auraData.spellId);
  end

  local name, iconTexture, _, _, duration, expirationTime, _, _, _, auraId = UnitBuff(unitID, auraIndex);
  return
    (NotSecretValue(name) and name) or nil,
    (NotSecretValue(iconTexture) and iconTexture) or nil,
    GetSafeNumericValue(duration),
    GetSafeNumericValue(expirationTime),
    GetSafeNumericValue(auraId);
end

GetSafeCastInfo = function(unitID, channelling)
  local name, text, texture, startTime, endTime, notInterruptible, numStages;

  if (channelling) then
    name, text, texture, startTime, endTime, _, notInterruptible, _, _, numStages = UnitChannelInfo(unitID);
  else
    name, text, texture, startTime, endTime, _, _, notInterruptible = UnitCastingInfo(unitID);
  end

  local safeStartTime = GetSafeNumericValue(startTime);
  local safeEndTime = GetSafeNumericValue(endTime);

  if (not (safeStartTime and safeEndTime)) then
    return;
  end

  return {
    name = NotSecretValue(name) and name or nil;
    text = NotSecretValue(text) and text or nil;
    texture = NotSecretValue(texture) and texture or nil;
    startTime = safeStartTime;
    endTime = safeEndTime;
    notInterruptible = (NotSecretValue(notInterruptible) and notInterruptible) or false;
    numStages = GetSafeNumericValue(numStages, 0) or 0;
  };
end
...
function Events:UNIT_AURA(castBar, castBarData, unitID)
  if (unitID ~= castBarData.unitID) then return end
  if (unitID == "mirror") then return end

  for auraID = 1, BUFF_MAX_DISPLAY do
    local name, iconTexture, duration, expirationTime, auraId = GetHelpfulAuraInfo(unitID, auraID);

    if (name and duration and expirationTime and ((auraId and tk.Constants.FOOD_DRINK_AURAS[tostring(auraId)]) or name == "Food" or name == "Drink")) then
...
function Events:UNIT_POWER_UPDATE(_, castBarData)
  ...
  if (not obj:IsNumber(max) or max <= 0) then
    self:UNIT_POWER_BAR_HIDE(nil, castBarData);
    return;
  end
...
local function IsFinished(data)
  ...
  if (data.auraId) then
    for auraID = 1, BUFF_MAX_DISPLAY do
      local auraId = select(5, GetHelpfulAuraInfo(data.unitID, auraID));
  ...
  if (self.enabled and self.totalElapsed > 0.01) then
    ...
    if (data.unitID == "mirror") then
      ...
      if (data.startTime and not IsFinished(data)) then
        local castInfo = GetSafeCastInfo(data.unitID, false);

        if (castInfo) then
          data.startTime = castInfo.startTime / 1000;
        else
          castInfo = GetSafeCastInfo(data.unitID, true);
...
do
  local function CreateBarFrame(unitID, settings, globalName)
    ...
    elseif (unitID == "player" and ShouldHideBlizzardCastBars()) then
      SuppressBlizzardCastBar(GetPlayerCastingBarFrame());
    elseif (unitID == "pet" and ShouldHideBlizzardCastBars()) then
      SuppressBlizzardCastBar(_G.PetCastingBarFrame);
    end
  ...
  function C_CastBar:SetEnabled(data, enabled)
    ...
    if (enabled) then
      ...
      if (data.unitID == "player" and ShouldHideBlizzardCastBars()) then
        SuppressBlizzardCastBar(GetPlayerCastingBarFrame());
      elseif (data.unitID == "pet" and ShouldHideBlizzardCastBars()) then
        SuppressBlizzardCastBar(_G.PetCastingBarFrame);
      end

      self:SetNameEnabled(data.settings.showName);
      self:SetIconEnabled(data.settings.showIcon);
...
function C_CastBar:SetTicks(data, numTicks)
  if (not (obj:IsTable(data) and obj:IsWidget(data.frame) and obj:IsWidget(data.frame.statusbar))) then
    return;
  end
...
obj:DefineParams("boolean");
function C_CastBar:SetNameEnabled(data, enabled)
  if (obj:IsWidget(data.frame) and obj:IsWidget(data.frame.name)) then
    data.frame.name:SetShown(enabled);
  end
end
...
function C_CastBar:StartCasting(data, channelling, fadeIn, auraInfo)
  ...
  if (not obj:IsTable(auraInfo)) then
    local castInfo = GetSafeCastInfo(data.unitID, channelling);

    if (castInfo) then
      name = castInfo.name;
      text = castInfo.text;
      texture = castInfo.texture;
      startTime = castInfo.startTime;
      endTime = castInfo.endTime;
      notInterruptible = castInfo.notInterruptible;
      numStages = castInfo.numStages or 0;
    end
  ...
  if (tk:IsRetail() and numStages) then
      ...
      local holdAtMaxTime = obj:IsFunction(_G.GetUnitEmpowerHoldAtMaxTime)
        and _G.GetUnitEmpowerHoldAtMaxTime(data.unitID) or 0;
			endTime = endTime + (obj:IsNumber(holdAtMaxTime) and holdAtMaxTime or 0);
      channelling = false;
		end
  end
  ...
  if (bar.latencyBar) then
    if (data.settings.showLatency) then
      ...
      if (obj:IsFunction(GetNetStats)) then
        local _, _, _, homeLatency = GetNetStats();

        if (obj:IsNumber(homeLatency)) then
          latencyMS = homeLatency;
        end
      end

      local percent = latencyMS / 1000;
...
function C_CastBar:PositionCastBar(data)
  if (not (obj:IsTable(data)
      ...
      and obj:IsWidget(data.frame))) then
    return;
  end
  ...
  if (not obj:IsWidget(sufAnchor)) then
    sufAnchor = nil;
  end
  local manualPosition = not (anchorToSUF and sufAnchor);

  if (manualPosition) then
    local position = obj:IsTable(data.settings.position) and data.settings.position or nil;
    local point = position and position[1];
    local relativeFrame = position and position[2];
    local relativePoint = position and position[3];
    local x, y = position and position[4], position and position[5];
    local relativeTo = obj:IsString(relativeFrame) and _G[relativeFrame] or nil;

    data.frame:SetParent(_G.UIParent);

    if (point and relativeFrame and relativePoint and x and y) then
      data.frame:SetPoint(point, relativeTo or _G.UIParent, relativePoint, x, y);
    else
...
function C_CastBar:CheckStatus(data)
  if (GetSafeCastInfo(data.unitID, false)) then
    self:StartCasting(false, false);
  elseif (GetSafeCastInfo(data.unitID, true)) then
...
function C_CastBarsModule:OnInitialize(data)
  ...
  local options = {
    ...
    groups = {
      ...
      {
        patterns = { "(width|height|frameStrata|frameLevel|showName|showIcon|leftToRight)" };
        value = function(value, keysList)
          ...
          elseif (attribute == "showName") then
            castBar:SetNameEnabled(value);
      ...
      appearance = {
        texture = function(value)
          ...
          for _, castBar in pairs(data.bars) do
            ...
            if (IsCastBarFrameReady(castBarData)) then
              ...
              if (texture) then
                texture:SetBlendMode(data.settings.appearance.blendMode);
              end
            end
          end
        end;
        ...
        blendMode = function(value)
          ...
          for _, castBar in pairs(data.bars) do
            castBarData = data:GetFriendData(castBar);
            local texture = IsCastBarFrameReady(castBarData)
              and castBarData.frame.statusbar:GetStatusBarTexture();
        ...
        border = function(value)
          ...
          for _, castBar in pairs(data.bars) do
            castBarData = data:GetFriendData(castBar);
            if (obj:IsTable(castBarData) and obj:IsTable(castBarData.backdrop)
                and obj:IsWidget(castBarData.frame)) then
        ...
        fontSize = function(value)
          ...
          for _, castBar in pairs(data.bars) do
            castBarData = data:GetFriendData(castBar);
            if (obj:IsTable(castBarData) and obj:IsTable(castBarData.backdrop)) then
              castBarData.backdrop.edgeSize = value;
            end

            if (obj:IsWidget(castBarData.frame)
                and obj:IsWidget(castBarData.frame.name)
                and obj:IsWidget(castBarData.frame.duration)) then
        ...
        colors = {
          background = function(value)
            ...
            for _, castBar in pairs(data.bars) do
              ...
              if (not IsCastBarFrameReady(castBarData)) then
                -- no-op
              elseif (not castBarData.background) then
          ...
          border = function(value)
            ...
            for _, castBar in pairs(data.bars) do
              castBarData = data:GetFriendData(castBar);
              if (obj:IsWidget(castBarData.frame)) then
          ...
          latency = function(value)
            ...
            if (castBar and data.settings.Player.showLatency) then
              ...
              if (obj:IsWidget(castBarData.frame) and obj:IsWidget(castBarData.frame.latencyBar)) then
