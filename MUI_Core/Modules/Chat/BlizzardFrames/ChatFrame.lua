...
--------------------------------------
local RETAIL_CHAT_SAFE_MODE = tk:IsRetail();
...
local function Tab_OnClick(self)
  ...
  if (obj:IsWidget(dock) and obj:IsFunction(_G.FCFDock_UpdateTabs)) then
    _G.FCFDock_UpdateTabs(dock);
  end
end
...
function C_ChatModule:SetUpBlizzardChatFrame(data, chatFrameName)
  ...
  if (not RETAIL_CHAT_SAFE_MODE) then
...
function C_ChatModule:SetUpAllBlizzardFrames()
  ...
  -- Retail uses protected FloatingChatFrame fade paths. Do not taint Blizzard globals.
  if (not RETAIL_CHAT_SAFE_MODE) then
