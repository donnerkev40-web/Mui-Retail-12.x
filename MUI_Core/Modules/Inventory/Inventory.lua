...
local InventoryAnchors = obj:Import("MayronUI.Inventory.AnchorController");
...
local internalTabBarInsetRight = 0;
local internalTabBarInsetTop = 0;
local IsInventoryFrameLocked = function(db)
  return InventoryAnchors:IsFrameLocked(db);
end;
local RepositionInventoryFrame = function(inventoryFrame)
  return InventoryAnchors:RepositionFrame(inventoryFrame);
end;
local StoreInventoryFrameOffsets = function(inventoryFrame, db)
  return InventoryAnchors:StoreFrameOffsets(inventoryFrame, db);
end;
local RestoreInventoryFramePosition = function(inventoryFrame, db)
  return InventoryAnchors:RestoreFramePosition(inventoryFrame, db);
end;
local EnsureInventoryFrameIsOnScreen = function(inventoryFrame)
  return InventoryAnchors:EnsureFrameIsOnScreen(inventoryFrame);
end;
local ApplyInventoryFrameLockState = function(inventoryFrame, db)
  return InventoryAnchors:ApplyLockState(inventoryFrame, db);
end;

-- Local Functions --------------

local function GetEffectiveContainerTopPadding()
  return containerPadding.top + internalTabBarInsetTop;
end
...
local function GetFrameDimensionsByTotalColumns(inventoryFrame, slotSpacing, totalColumns, slotHeight, slotWidth, totalRows, maxRows)
  ...
  local newWidth = containerPadding.left + bagsContainerWidth + containerPadding.right + internalTabBarInsetRight;
  local newHeight = GetEffectiveContainerTopPadding() + bagsContainerHeight + containerPadding.bottom;
...
local function SetDetailedViewEnabled(inventoryFrame, enabled)
  ...
  inventoryFrame.bagsContainer:SetPoint("TOPLEFT", containerPadding.left, -GetEffectiveContainerTopPadding());
  inventoryFrame.bagsContainer:SetPoint("BOTTOMRIGHT", -(containerPadding.right + internalTabBarInsetRight), containerPadding.bottom);
...
local function ToggleCharactersMenu(self)
  ...
  ShowDropDownMenu(self.menuList, self.menuFrame, self);
end
...
local function SetPadding(customPadding)
  ...
  local inventoryFrame = _G["MUI_Inventory"]--[[@as MayronUI.Inventory.Frame]];
  inventoryFrame.bagBar:SetPoint("TOPLEFT", containerPadding.left, -GetEffectiveContainerTopPadding());
  inventoryFrame.bagsContainer:SetPoint("TOPLEFT", containerPadding.left, -GetEffectiveContainerTopPadding());
  inventoryFrame.bagsContainer:SetPoint("BOTTOMRIGHT", -(containerPadding.right + internalTabBarInsetRight), containerPadding.bottom);
end
...
local function CreateOrUpdateTabBar(settings)
  ...
  if (not showTabBar) then
    internalTabBarInsetRight = 0;
    internalTabBarInsetTop = 0;
    inventoryFrame.bagBar:SetPoint("TOPLEFT", containerPadding.left, -GetEffectiveContainerTopPadding());
    inventoryFrame.bagsContainer:SetPoint("TOPLEFT", containerPadding.left, -GetEffectiveContainerTopPadding());
    inventoryFrame.bagsContainer:SetPoint("BOTTOMRIGHT", -(containerPadding.right + internalTabBarInsetRight), containerPadding.bottom);
  ...
  local isEmbeddedTopRail = position == "TOP" and direction == "RIGHT";
  if (isEmbeddedTopRail) then
    internalTabBarInsetTop = math.max(24, math.floor((26 * scale) + 10));
    internalTabBarInsetRight = 0;
  else
    internalTabBarInsetTop = 0;
    internalTabBarInsetRight = 0;
  ...
  for i = 1, 5 do
    ...
    if (tabEnabled or tab) then
      if (not tab) then
        tab = gui:CreateIcon(
          2, 30, 26, inventoryFrame, "check-button", nil, nil,
      ...
      if (tabEnabled) then
        ...
        if (not previousTab) then
          if (position == "TOP") then
            if (isEmbeddedTopRail) then
              tab:SetPoint("TOPLEFT", inventoryFrame, "TOPLEFT", xOffset, yOffset);
...
local function IsInventoryReplacementEnabled(db)
  return db and db.global:QueryType("boolean", "enabled");
end

local function ShowInventoryReplacement(inventoryFrame, db)
  if (IsInventoryReplacementEnabled(db) and inventoryFrame and not inventoryFrame:IsShown()) then
    inventoryFrame:Show();
  end
end

local function HideInventoryReplacement(inventoryFrame)
  if (inventoryFrame and inventoryFrame:IsShown()) then
    inventoryFrame:Hide();
  end
end

local function HookBagOpenFunction(functionName, inventoryFrame, db)
  if (obj:IsFunction(_G[functionName])) then
    tk:HookFunc(functionName, function()
      ShowInventoryReplacement(inventoryFrame, db);
    end);
  end
end

local function HookBagCloseFunction(functionName, inventoryFrame)
  if (obj:IsFunction(_G[functionName])) then
    tk:HookFunc(functionName, function()
      HideInventoryReplacement(inventoryFrame);
    end);
  end
end

local function ReplaceBagFunctions(inventoryFrame, db)
  if (not originalBagFunctions) then
  ...
  local function UseReplacement()
    return IsInventoryReplacementEnabled(db);
  end
  ...
  _G.ToggleAllBags = function(...)
    ...
    if (originalBagFunctions.ToggleAllBags) then
      return originalBagFunctions.ToggleAllBags(...);
    end
  end
  ...
  _G.OpenAllBags = function(...)
    ...
    if (originalBagFunctions.OpenAllBags) then
      return originalBagFunctions.OpenAllBags(...);
    end
  end
  ...
  _G.CloseAllBags = function(...)
    ...
    if (originalBagFunctions.CloseAllBags) then
      return originalBagFunctions.CloseAllBags(...);
    end
  end

  if (originalBagFunctions.ToggleBackpack) then
    _G.ToggleBackpack = function(...)
      ...
      return originalBagFunctions.ToggleBackpack(...);
    end
  end

  if (originalBagFunctions.OpenBackpack) then
    _G.OpenBackpack = function(...)
      ...
      return originalBagFunctions.OpenBackpack(...);
    end
  end

  if (originalBagFunctions.CloseBackpack) then
    _G.CloseBackpack = function(...)
      ...
      return originalBagFunctions.CloseBackpack(...);
    end
  end

  if (originalBagFunctions.ToggleBag) then
    _G.ToggleBag = function(...)
      ...
      return originalBagFunctions.ToggleBag(...);
    end
  end

  if (originalBagFunctions.OpenBag) then
    _G.OpenBag = function(...)
      if (UseReplacement()) then
        PlaySound(SoundKit.IG_BACKPACK_OPEN);
        ShowInventoryReplacement(inventoryFrame, db);
      ...
      return originalBagFunctions.OpenBag(...);
    end
  end

  if (originalBagFunctions.CloseBag) then
    _G.CloseBag = function(...)
      ...
      return originalBagFunctions.CloseBag(...);
    end
  end
end
...
function C_InventoryModule:OnInitialize()
  OrbitusDB:Register(addOnName, databaseConfig, function (db)
    ...
    do
      ...
      local scale = db.profile:QueryType("number", "container.tabBar.scale");

      if ((position == "RIGHT" and direction == "DOWN" and xOffset == 10
          and yOffset == 3 and spacing == 5 and scale == 1)
          or (position == "RIGHT" and direction == "DOWN" and xOffset == -8
          and yOffset == -38 and spacing == 3 and scale == 0.82)) then
        db.profile:Store("container.tabBar.position", "TOP");
        db.profile:Store("container.tabBar.direction", "RIGHT");
        db.profile:Store("container.tabBar.xOffset", 8);
        db.profile:Store("container.tabBar.yOffset", -34);
        db.profile:Store("container.tabBar.spacing", 4);
        db.profile:Store("container.tabBar.scale", 0.82);
      end
    end
    ...
    inventoryFrame:SetClampedToScreen(true);
    inventoryFrame:Hide();

    ReplaceBagFunctions(inventoryFrame, db);

    local reapplyBagReplacement = em:CreateEventListener(function()
      ReplaceBagFunctions(inventoryFrame, db);
    end);

    reapplyBagReplacement:RegisterEvents("PLAYER_ENTERING_WORLD", "ADDON_LOADED");

    if (C_Timer and C_Timer.After) then
      for _, delay in ipairs({1, 5, 15, 30, 60}) do
    ...
    tk:HookFunc("ToggleAllBags", function()
      if (not IsInventoryReplacementEnabled(db)) then
        return;
      end
    ...
    HookBagOpenFunction("OpenAllBags", inventoryFrame, db);
    HookBagOpenFunction("OpenBackpack", inventoryFrame, db);
    HookBagOpenFunction("OpenBag", inventoryFrame, db);

    if (obj:IsFunction(_G.ToggleBackpack)) then
      tk:HookFunc("ToggleBackpack", function()
        if (not IsInventoryReplacementEnabled(db)) then
          return;
        end
    ...
    HookBagCloseFunction("CloseAllBags", inventoryFrame);
    HookBagCloseFunction("CloseBackpack", inventoryFrame);
    HookBagCloseFunction("CloseBag", inventoryFrame);
    ...
    for i = 1, TOTAL_BAG_FRAMES do
      ...
      if (obj:IsWidget(blizzBag)) then
        blizzBag:HookScript("OnShow", function(self)
          if (not IsInventoryReplacementEnabled(db)) then
            return;
          end
    ...
    if (tk:IsRetail()) then
      ...
      if (obj:IsWidget(combinedBags)) then
        combinedBags:HookScript("OnShow", function(self)
          if (not IsInventoryReplacementEnabled(db)) then
            return;
          end
    ...
    for i = 0, (TOTAL_BAG_FRAMES - 1) do
      ...
      if (bagIndex == BagIndexes.Keyring) then
        bagFrame:Hide();
      end
    ...
    inventoryFrame.titleBar.onDragStart = function()
      if (IsInventoryFrameLocked(db)) then
        return;
      end
    ...
    inventoryFrame.titleBar.onDragStop = function()
      ...
      RepositionInventoryFrame(inventoryFrame);
      StoreInventoryFrameOffsets(inventoryFrame, db);
      blocker:Hide();
    end
    ...
    ApplyInventoryFrameLockState(inventoryFrame, db);

    inventoryFrame:SetScript("OnShow", function(self)
      EnsureInventoryFrameIsOnScreen(self);
      UpdateInventoryFrameCoreProperties(self);
      EnsureInventoryFrameIsOnScreen(self);
    end);
    ...
    RestoreInventoryFramePosition(inventoryFrame, db);
  end);
end
...
function C_InventoryModule:RegisterObservers(_, db)
  ...
  db.profile:Subscribe("container.locked", function()
    ApplyInventoryFrameLockState(inventoryFrame, db);
    StoreInventoryFrameOffsets(inventoryFrame, db);
    RepositionInventoryFrame(inventoryFrame);
  end);

  db.profile:Subscribe("container.tabBar", function(settings)
    CreateOrUpdateTabBar(settings);
    UpdateInventoryFrameCoreProperties(inventoryFrame, false);
  end);
