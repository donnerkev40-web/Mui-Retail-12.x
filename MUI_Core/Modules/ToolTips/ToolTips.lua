...
local TooltipStyleHelper = obj:Import("MayronUI.Tooltips.Style");
local TooltipAnchors = obj:Import("MayronUI.Tooltips.Anchors");
...
local function EnsureTooltipBackdropSupport(tooltip)
  return TooltipStyleHelper:EnsureBackdropSupport(tooltip);
end
...
local DISABLE_CUSTOM_TOOLTIP_STATUSBARS_ON_RETAIL = tk:IsRetail();
...
local function SetBackdropStyle(d)
  TooltipStyleHelper:ApplyBackdrop(d, tooltipStyle, gameTooltip, tooltipsToReskin);
end
...
local function GetAuraDataByIndex(unitID, index, filter)
  if (not (tk:IsRetail() and C_UnitAuras and C_UnitAuras.GetAuraDataByIndex)) then
    return;
  end

  local ok, auraData = _G.pcall(C_UnitAuras.GetAuraDataByIndex, unitID, index, filter);
...
local function GetTooltipAuraInfo(unitID, index, filter)
  local auraData = GetAuraDataByIndex(unitID, index, filter);

  if (auraData) then
    local auraType = auraData.dispelName or auraData.debuffType;
    local isFromPlayerOrPet = auraData.isFromPlayerOrPlayerPet;

    return
      NotSecretValue(auraData.name) and auraData.name or nil,
      NotSecretValue(auraData.icon) and auraData.icon or nil,
      NotSecretValue(auraType) and auraType or nil,
      GetSafeNumericValue(auraData.duration, 0),
      GetSafeNumericValue(auraData.expirationTime, 0),
      (NotSecretValue(isFromPlayerOrPet) and isFromPlayerOrPet == true) or false;
  end

  local auraName, iconTexture, _, auraType, duration, endTime, source = UnitAura(unitID, index, filter);

  return
    (NotSecretValue(auraName) and auraName) or nil,
    (NotSecretValue(iconTexture) and iconTexture) or nil,
    (NotSecretValue(auraType) and auraType) or nil,
    GetSafeNumericValue(duration, 0),
    GetSafeNumericValue(endTime, 0),
    (NotSecretValue(source) and source == "player") or false;
end

local function SafeDivide(numerator, denominator, fallback)
  local safeDenominator = GetSafeNumericValue(denominator, 0);
  if (safeDenominator <= 0) then
    return fallback or 0;
  end

  local safeNumerator = GetSafeNumericValue(numerator, 0);
...
local function SetTooltipAccentColor(tooltip, useMuiTexture, r, g, b)
  TooltipStyleHelper:SetAccentColor(tooltip, useMuiTexture, r, g, b);
end

local function GetHealthColor(healthColors, currentHealth, maxHealth)
  local percent = SafeDivide(currentHealth, maxHealth, 0);
...
do
  ...
  function UpdateUnitHealthBarText(format, healthColors, unitID)
    local maxHealth = GetSafeNumericValue(UnitHealthMax(unitID), 0);
    local currentHealth = GetSafeNumericValue(UnitHealth(unitID), 0);
    local r, g, b = GetHealthColor(healthColors, currentHealth, maxHealth);
    local percentage = SafeDivide(currentHealth, maxHealth, 0) * 100;
...
local function SetFonts(data)
  ...
  local function SetFontIfPossible(fontString, fontPath, fontSize, fontFlag)
    if (obj:IsTable(fontString) and obj:IsFunction(fontString.SetFont)) then
      fontString:SetFont(fontPath, fontSize, fontFlag);
    end
  end

  local function SetShoppingTooltipFonts(tooltip)
    if (not obj:IsWidget(tooltip)) then
      return;
    end

    local tooltipName = tooltip:GetName();
    if (not obj:IsString(tooltipName)) then
      return;
    end
    ...
    for i = 1, tooltip:NumLines() do
      ...
      local leftSize = (i == 1 and (size + 2)) or size;
      SetFontIfPossible(left, font, leftSize, flag);
      SetFontIfPossible(right, font, size - 2, flag);
    end
  end

  SetShoppingTooltipFonts(_G.ShoppingTooltip1);
  SetShoppingTooltipFonts(_G.ShoppingTooltip2);
...
local function UpdateUnitTexture(d, unitID)
  ...
  if (UnitIsPlayer(unitID)) then
    ...
    SetTooltipAccentColor(gameTooltip, settings.useMuiTexture, r, g, b);
  ...
  if (settings.useMuiTexture) then
    ...
    SetTooltipAccentColor(gameTooltip, true, r, g, b);
    return
  end
  ...
  SetTooltipAccentColor(gameTooltip, false, r, g, b);
end

local function UpdateUnitStatusBars(d, unitID)
  if (DISABLE_CUSTOM_TOOLTIP_STATUSBARS_ON_RETAIL) then
    -- Retail taint hardening:
    -- Disable custom tooltip health/power extras and keep base tooltip stable.
...
local function AddAuras(data, filter, anchor, unitID)
  ...
  local maxWidth = GetSafeNumericValue(gameTooltip:GetWidth(), 250);
  ...
  for index = 1, maxAuras do
    local auraName, iconTexture, auraType, duration, endTime, isOwnedByPlayer = GetTooltipAuraInfo(unitID, index, filter);
    local show = true;

    if (settings.onlyYours and not isOwnedByPlayer) then
      show = false;
    end
    ...
    if (auraName and iconTexture and show) then
      ...
      if (not frame) then
        auraFrames[id] = tk:CreateFrame("Frame", gameTooltip);
        frame = auraFrames[id];
        local edge = tk.Constants.BACKDROP.edgeSize;

        frame.borderTexture = frame:CreateTexture(nil, "BACKGROUND");
        frame.borderTexture:SetAllPoints(true);
        frame.borderTexture:SetColorTexture(0, 0, 0, 0);
      ...
      if (filter == HARMFUL) then
        ...
        if (obj:IsTable(frame.borderTexture) and obj:IsFunction(frame.borderTexture.SetColorTexture)) then
          frame.borderTexture:SetColorTexture(unpack(auraColor));
        end
      else
        if (obj:IsTable(frame.borderTexture) and obj:IsFunction(frame.borderTexture.SetColorTexture)) then
          frame.borderTexture:SetColorTexture(unpack(data.settings.colors.buff));
        end
      end
...
local function CreateScreenAnchor(data)
  TooltipAnchors:CreateScreenAnchor(data);
end

local function ShouldBeHidden(data, tooltip)
  return TooltipAnchors:ShouldBeHidden(data, tooltip);
end
...
function C_ToolTipsModule:OnEnable(data)
  ...
  if (not data.gameTooltipHooksInstalled) then
    gameTooltip:HookScript("OnShow", function()
      ...
      if (not gameTooltip:GetUnit()) then
        HideAllAuras(data);
        local r, g, b = gui:GetMuiFrameColor();
        SetTooltipAccentColor(gameTooltip, data.settings.useMuiTexture, r, g, b);
      end
    ...
    gameTooltip:HookScript("OnUpdate", function(self, elapsed)
      ...
      if (data.lastUpdated > 0.2) then
        ...
        if (not DISABLE_CUSTOM_TOOLTIP_STATUSBARS_ON_RETAIL) then
    ...
    data.gameTooltipHooksInstalled = true;
  end

  if (not data.defaultAnchorHookInstalled) then
    hooksecurefunc("GameTooltip_SetDefaultAnchor", function(self, parent)
      TooltipAnchors:ApplyDefaultAnchor(data, self, parent);
    end);

    data.defaultAnchorHookInstalled = true;
  end
  ...
  if (not data.unitTooltipHookInstalled) then
    ...
    data.unitTooltipHookInstalled = true;
  end

  if (not data.inspectListener) then
    data.inspectListener = em:CreateEventListener(function(_, _, unitGuid)
      data.inspecting = unitGuid;
    end);
    data.inspectListener:RegisterEvent("INSPECT_READY");
  end

  if (not data.cacheCleanupFrame) then
    data.cacheCleanupFrame = tk:CreateFrame("Frame");
    data.cacheCleanupFrame:SetScript("OnUpdate", function(self, elapsed)
