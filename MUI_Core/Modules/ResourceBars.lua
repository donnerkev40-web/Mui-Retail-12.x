...
local function ApplyVisualUpdates(_, _, container, bars)
  if (InCombatLockdown() or not (obj:IsWidget(container) and obj:IsTable(bars))) then
    return;
  end
  ...
  for _, barName in ipairs(BAR_NAMES) do
    ...
    if (bars[barName]) then
      ...
      local frame = obj:IsFunction(bar.GetFrame) and bar:GetFrame();

      -- check if frame has been frame (bar has been built)
      if (frame and obj:IsFunction(bar.IsEnabled) and obj:IsFunction(bar.CanUse)
          and bar:IsEnabled() and bar:CanUse()) then
  ...
  if (obj:IsFunction(container.SetHeight)) then
...
function C_ResourceBarsModule:OnInitialize(data, containerModule)
  ...
  local function UpdateResourceBar(_, keysList)
    if (not (obj:IsTable(data.bars) and obj:IsTable(keysList))) then
      return;
    end
    ...
    if (settingName == "texture") then
      if (obj:IsFunction(bar.UpdateStatusBarTexture)) then
        bar:UpdateStatusBarTexture();
      end
    else
      if (obj:IsFunction(bar.Update)) then
        bar:Update();
      end
    end

    if (settingName == "height" and em:GetEventListenerByID("ResourceBars_ApplyVisualUpdate")) then
  ...
  local updateFuncs = {
    experienceBar = {
      enabled = function(value)
        if (obj:IsTable(data.bars) and data.bars.experience and obj:IsFunction(data.bars.experience.SetEnabled)) then
    ...
    reputationBar = {
      enabled = function(value)
        if (obj:IsTable(data.bars) and data.bars.reputation and obj:IsFunction(data.bars.reputation.SetEnabled)) then
  ...
  if (tk:IsRetail()) then
    updateFuncs.artifactBar = {
      enabled = function(value)
        if (obj:IsTable(data.bars) and data.bars.artifact and obj:IsFunction(data.bars.artifact.SetEnabled)) then
    ...
    updateFuncs.azeriteBar = {
      enabled = function(value)
        if (obj:IsTable(data.bars) and data.bars.azerite and obj:IsFunction(data.bars.azerite.SetEnabled)) then
...
function C_ResourceBarsModule:OnEnable(data)
  ...
  if (_G.BackdropTemplateMixin) then
    _G.Mixin(data.barsContainer, _G.BackdropTemplateMixin);
    data.barsContainer:OnBackdropLoaded();
    data.barsContainer:SetScript("OnSizeChanged", data.barsContainer.OnBackdropSizeChanged);
  end

  if (obj:IsFunction(data.barsContainer.SetBackdrop)) then
    data.barsContainer:SetBackdrop(tk.Constants.BACKDROP_WITH_BACKGROUND);
    data.barsContainer:SetBackdropBorderColor(0, 0, 0);
    data.barsContainer:SetBackdropColor(0.06, 0.06, 0.06, 0.85);
  end
...
function C_ResourceBarsModule:SetBlockerEnabled(data, enabled, dataTextBar)
  ...
  if (not data.blocker and enabled) then
    ...
    if (obj:IsWidget(dataTextBar)) then
      data.blocker:SetPoint("BOTTOMRIGHT", dataTextBar, "BOTTOMRIGHT");
    else
      data.blocker:SetPoint("BOTTOMRIGHT");
    end
