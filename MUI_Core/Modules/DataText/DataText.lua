...
local C_DataTextModule = MayronUI:RegisterModule("DataTextModule", "Info Panel");
  ...
  self:RegisterUpdateFunctions(db.profile.datatext, {
    ...
    fontSize = function(value)
      for _, btn in ipairs(data.buttons) do
        local muiFont = tk:GetMuiFont();
        btn:GetFontString():SetFont(muiFont, value, "");
      end
    end;
...
function C_DataTextModule:CreateDataTextButton(data)
  ...
  local muiFont = tk:GetMuiFont();
  btn:GetFontString():SetFont(muiFont, data.settings.fontSize, "");
