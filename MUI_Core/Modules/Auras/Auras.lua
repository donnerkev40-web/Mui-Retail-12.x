...
if (not obj:IsFunction(UnitAura)) then
  ...
  if (C_UnitAuras and C_UnitAuras.GetAuraDataByIndex) then
    local function SafeGetAuraDataByIndex(unitID, index, filter)
      if (type(unitID) ~= "string" or unitID == "") then
        return;
      end

      local ok, auraData = pcall(C_UnitAuras.GetAuraDataByIndex, unitID, index, filter);
    ...
    UnitAura = function(unitID, indexOrName, filter)
      local auraData;

      if (type(indexOrName) == "number") then
        auraData = SafeGetAuraDataByIndex(unitID, indexOrName, filter);
      elseif (type(indexOrName) == "string") then
        auraData = SafeFindAuraByName(indexOrName, unitID, filter);
      end
...
local ORIGINAL_AURA_STATUSBAR_ANCHORS = {
  buffs = {
    relFrame = "UIParent";
    relPoint = "TOPRIGHT";
    xOffset = -3;
...
local function GetSafeArithmeticResult(callback, fallback)
  local ok, result = _G.pcall(callback);
...
local function GetAuraDataByIndex(unitID, auraIndex, filter)
  if (not (tk:IsRetail() and C_UnitAuras and C_UnitAuras.GetAuraDataByIndex)) then
    return;
  end

  local ok, auraData = _G.pcall(C_UnitAuras.GetAuraDataByIndex, unitID, auraIndex, filter);
...
local function GetAuraInfo(unitID, auraIndex, filter)
  local auraData = GetAuraDataByIndex(unitID, auraIndex, filter);

  if (auraData) then
    local auraSubType = auraData.dispelName or auraData.debuffType;
    local applications = auraData.applications or auraData.charges or auraData.stackCount;
    ...
    return auraData,
      ...
      (NotSecretValue(auraSubType) and auraSubType) or nil,
      GetSafeAuraTimeValue(auraData.duration, 0),
      GetSafeAuraTimeValue(auraData.expirationTime, 0),
      NotSecretValue(auraData.sourceUnit) and auraData.sourceUnit or nil;
  end

  local name, texture, count, auraSubType, duration, expiryTime, source = UnitAura(unitID, auraIndex, filter);

  return nil,
    (NotSecretValue(name) and name) or nil,
    (NotSecretValue(texture) and texture) or nil,
    GetSafeAuraCountValue(count),
    (NotSecretValue(auraSubType) and auraSubType) or nil,
    GetSafeAuraTimeValue(duration, 0),
    GetSafeAuraTimeValue(expiryTime, 0),
    (NotSecretValue(source) and source) or nil;
end

local function IsAuraOwnedByPlayer(auraData, source)
  if (auraData) then
    local isFromPlayer = auraData.isFromPlayerOrPlayerPet;

    if (NotSecretValue(isFromPlayer)) then
      return isFromPlayer == true;
    end

    local sourceUnit = auraData.sourceUnit;
    if (NotSecretValue(sourceUnit) and obj:IsString(sourceUnit)) then
      return (sourceUnit == "player" or sourceUnit == "pet" or sourceUnit == "vehicle");
    end
  end

  if (source and NotSecretValue(source)) then
    local ok, owned = _G.pcall(function()
      return UnitGUID("player") == UnitGUID(source);
    end);
...
local function RestoreOriginalStatusbarAnchors(db)
  for auraType, original in pairs(ORIGINAL_AURA_STATUSBAR_ANCHORS) do
    local relFrame = db.profile:QueryType("string", auraType, "statusbars", "relFrame");

    if (relFrame == "Minimap") then
      db.profile:Store(auraType .. ".statusbars.relFrame", original.relFrame);
      db.profile:Store(auraType .. ".statusbars.relPoint", original.relPoint);
      db.profile:Store(auraType .. ".statusbars.xOffset", original.xOffset);
      db.profile:Store(auraType .. ".statusbars.yOffset", original.yOffset);
    end
  end
end
...
local function HandleAuraButtonOnUpdate(self, elapsed)
  ...
  self.timeRemaining = GetSafeArithmeticResult(function()
    return self.expiryTime - GetTime();
  end, 0);
  ...
  if (self.countText and self.countLastUpdate > 0.1) then
    ...
    local auraData, _, _, count = GetAuraInfo("player", id, self.filter);
...
local function OnAuraButtonAttributeChanged(self, attribute, value)
  ...
  if (attribute == "index") then
    local auraData, name, texture, count, auraSubType, safeDuration, safeExpiryTime, source =
      GetAuraInfo("player", value, self.filter);

    self.expiryTime = safeExpiryTime;
    self.startTime = GetSafeArithmeticResult(function()
      return safeExpiryTime - safeDuration;
    end, 0);
    self.duration = safeDuration; -- can use cooldown
    self.timeRemaining = GetSafeArithmeticResult(function()
      return safeExpiryTime - GetTime();
    end, 0);
    self.auraSubType = auraSubType;
    self.count = count;
    self.name = name;
    self.owned = IsAuraOwnedByPlayer(auraData, source);
...
local function CreateOrUpdateAuraHeader(filter, db, header)
  ...
  if (not header.__muiOnAttributeChangedHookInstalled) then
    header:HookScript("OnAttributeChanged", OnHeaderAttributeChanged);
    header.__muiOnAttributeChangedHookInstalled = true;
  end
...
function C_AurasModule:OnEnabled(data)
  ...
  RestoreOriginalStatusbarAnchors(db);
