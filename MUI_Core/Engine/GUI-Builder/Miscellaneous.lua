...
local function IsConfigContext(frame)
  local current = frame;

  while (current) do
    if (current == _G.MUI_Config) then
      return true;
    end

    if (obj:IsFunction(current.GetName) and current:GetName() == "MUI_Config") then
      return true;
    end

    current = obj:IsFunction(current.GetParent) and current:GetParent() or nil;
  end

  return false;
end

local function GetHighlightFontObject(frame)
  if (IsConfigContext(frame)) then
    return "MUI_ConfigFont";
  end

  return "GameFontHighlight";
end

local function GetDisabledFontObject(frame)
  if (IsConfigContext(frame)) then
    return "MUI_ConfigFontDisabled";
  end

  return "GameFontDisable";
end

local function GetLargeFontObject(frame)
  if (IsConfigContext(frame)) then
    return "MUI_ConfigFontLarge";
  end
...
local function OnGridFrameSizeChanged(self)
  ...
  if (not (NotSecretValue(width) and NotSecretValue(height))) then
    return;
  end
...
do
  ...
  function gui:CreateButton(parent, text, button, tooltip, padding, minWidth)
    ...
    local normalFontObject = GetHighlightFontObject(button);
    local disabledFontObject = GetDisabledFontObject(button);
    local fs = button:CreateFontString(nil, "OVERLAY", normalFontObject)--[[@as FontString]];
    ...
    button:SetNormalFontObject(normalFontObject);
    button:SetDisabledFontObject(disabledFontObject);
...
do
  local function OnCheckButtonSetEnabled(self, value)
    if (value) then
      self.text:SetFontObject(GetHighlightFontObject(self));
    else
      self.text:SetFontObject(GetDisabledFontObject(self));
    end
  end
  ...
  function gui:CreateCheckButton(parent, text, tooltip, globalName, verticalAlignment, radio, isSwatch)
    ...
    container.btn.text:SetFontObject(GetHighlightFontObject(container));
...
do
  ...
  function gui:AddTitleBar(frame, text)
    ...
    frame.titleBar.text = frame.titleBar:CreateFontString(nil, "ARTWORK", GetLargeFontObject(frame));
