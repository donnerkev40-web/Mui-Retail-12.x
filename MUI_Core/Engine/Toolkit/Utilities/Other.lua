...
local function GetResolvedItemLevel(itemLink)
  ...
  if (obj:IsFunction(GetDetailedItemLevelInfo)) then
    itemLevel = GetDetailedItemLevelInfo(itemLink);
  end

  if (not obj:IsNumber(itemLevel)) then
    local _, _, _, cachedItemLevel = GetItemInfo(itemLink);
    itemLevel = cachedItemLevel;
  end

  if (not obj:IsNumber(itemLevel) and obj:IsTable(C_Item)
      and obj:IsFunction(C_Item.GetDetailedItemLevelInfo)) then
    itemLevel = C_Item.GetDetailedItemLevelInfo(itemLink);
  end

  if (obj:IsNumber(itemLevel) and itemLevel > 0) then
    return itemLevel;
  end
end
...
function tk:GetInspectItemLevel(unitID)
  ...
  if (GetInspectItemLevel) then
    local inspectItemLevel = GetInspectItemLevel(unitID);

    if (obj:IsNumber(inspectItemLevel) and inspectItemLevel > 0) then
  ...
  for _, slotID in ipairs(ITEM_GEAR_SLOTS) do
    ...
    if (itemLink) then
      local itemLevel = GetResolvedItemLevel(itemLink);

      if (itemLevel) then
        resolvedSlots = resolvedSlots + 1;
  ...
  if (resolvedSlots == 0) then
    if (unitID == "player" and obj:IsFunction(_G.GetAverageItemLevel)) then
      local _, equippedItemLevel = _G.GetAverageItemLevel();

      if (obj:IsNumber(equippedItemLevel) and equippedItemLevel > 0) then
        return tk.Numbers:ToPrecision(equippedItemLevel, 1);
      end
    end
  ...
  local average = total / resolvedSlots;
...
do
    ...
    local function GetPopupButton(popup, buttonID)
        if (not obj:IsTable(popup)) then
          return;
        end

        local suffix = tostring(buttonID);
        local button = popup["button" .. suffix];

        if (obj:IsTable(button)) then
          return button;
        end

        button = popup["Button" .. suffix];

        if (obj:IsTable(button)) then
          return button;
        end

        if (obj:IsTable(popup.Buttons)) then
          button = popup.Buttons[buttonID];
        ...
        if (obj:IsFunction(popup.GetName)) then
          local popupName = popup:GetName();

          if (popupName) then
            button = _G[popupName .. "Button" .. suffix];
    ...
    local function EditBox_OnTextChanged(self, userInput)
      ...
      local button1 = GetPopupButton(self.popup, 1);

      if (button1 and obj:IsFunction(button1.SetEnabled)) then
        button1:SetEnabled(isValid);
      end
    end

    local function PopUp_OnShow(self)
      local button1 = GetPopupButton(self, 1);
      local button2 = GetPopupButton(self, 2);

      if (button1 and obj:IsFunction(button1.Enable)) then
        button1:Enable();
      end

      if (button2 and obj:IsFunction(button2.Enable)) then
        button2:Enable();
      end

      self.editBox = GetPopupEditBox(self);

      if (not self.editBox or not self.editBox:IsShown()) then
        if (button1 and obj:IsFunction(button1.SetHeight)) then
          button1:SetHeight(30);
        end

        if (button2 and obj:IsFunction(button2.SetHeight)) then
          button2:SetHeight(30);
        end

        if (self.numButtons == 1 and button1 and obj:IsFunction(button1.GetTextWidth)) then
          local width = button1:GetTextWidth();

          if (width > 200) then
            width = width + 40;
            button1:SetWidth(width);
            local point, relFrame, relPoint, _, y = button1:GetPoint();

            local offset = width / 2;
            button1:SetPoint(point, relFrame, relPoint, -offset, y);
          end
        end
        return
      end
    ...
    local function ShowConfirmPopup(message, subMessage, confirmText, onConfirm, cancelText, onCancel, isWarning, ...)
      ...
      popup.showAlert = isWarning or false;
...
local function ParseVersionTriplet(versionText)
  if (not obj:IsString(versionText)) then
    return 0, 0, 0;
  end

  local version = versionText:match("(%d+%.%d+%.%d+)");

  if (not version) then
    return 0, 0, 0;
  end

  local major, minor, patch = tk.Strings:Split(version, ".");
...
function tk:GetTutorialShowState(oldVersion, afterInstall)
  ...
  local major, minor, patch = ParseVersionTriplet(currentVersion);
  ...
  if (not oldVersion or not (obj:IsString(oldVersion))) then
    ...
    local oldMajor, oldMinor, oldPatch = ParseVersionTriplet(oldVersion);
...
function tk:GetVersion(colorKey, raw)
  ...
  local function AppendClient(versionText)
    if (not obj:IsString(versionText)) then
      return versionText;
    end

    if (versionText:find("RETAIL", nil, true)
        or versionText:find("CLASSIC", nil, true)
        or versionText:find("WRATH", nil, true)
        or versionText:find("BCC", nil, true)) then
      return versionText;
    end

    return string.format("%s%s", versionText, client);
  end

  if (muiCore == muiConfig and muiConfig == muiSetup) then
     version = AppendClient(muiCore);
  else
    muiCore = AppendClient(muiCore);
    muiConfig = AppendClient(muiConfig);
    muiSetup = AppendClient(muiSetup);
