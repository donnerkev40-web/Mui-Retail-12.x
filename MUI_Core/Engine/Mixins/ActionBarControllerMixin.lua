...
local function RefreshBartenderBar(bt4Bar)
  if (not obj:IsTable(bt4Bar)) then
    return;
  end

  if (obj:IsFunction(bt4Bar.ApplyConfig)) then
    bt4Bar:ApplyConfig();
    return;
  end

  if (obj:IsFunction(bt4Bar.LoadPosition)) then
    bt4Bar:LoadPosition();
  end

  if (obj:IsFunction(bt4Bar.SetConfigScale)) then
    bt4Bar:SetConfigScale();
  end

  if (obj:IsFunction(bt4Bar.SetConfigAlpha)) then
    bt4Bar:SetConfigAlpha();
  end

  if (obj:IsFunction(bt4Bar.InitVisibilityDriver)) then
    bt4Bar:InitVisibilityDriver();
  end
end
  ...
  if (controller.bartender.controlVisibility) then
    for setId = 1, controller.animation.activeSets do
      ...
      for _, bt4Bar in pairs(controller.sets[setId]) do
        local isHidden = bt4Bar:GetVisibilityOption("always");
        local hasZeroConfigAlpha = obj:IsFunction(bt4Bar.GetConfigAlpha) and bt4Bar:GetConfigAlpha() == 0;
        local hasZeroAlpha = obj:IsFunction(bt4Bar.GetAlpha) and bt4Bar:GetAlpha() == 0;
        local isShown = not obj:IsFunction(bt4Bar.IsShown) or bt4Bar:IsShown();

        if (isHidden or hasZeroConfigAlpha or hasZeroAlpha or not isShown) then
          bt4Bar:SetConfigAlpha(1);
          bt4Bar:SetVisibilityOption("always", false);
          RefreshBartenderBar(bt4Bar);
          shouldFade = true;
        end
      end
